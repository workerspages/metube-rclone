...
# 常用环境变量
#  MeTube 存放持久化状态文件（如队列、历史、订阅配置等 json 文件）的路径。Docker 中默认为 /downloads/.metube
ENV STATE_DIR=/data/metube/.metube
# 中间下载文件（缓存/临时文件）的保存路径。Docker 中默认为 /downloads。建议将其映射到 SSD 或内存盘 (tmpfs) 以提升性能
ENV TEMP_DIR=/tmp/metube_temp
# 允许同时进行的最大下载数量
...
ENV CLEAR_COMPLETED_AFTER=259200
